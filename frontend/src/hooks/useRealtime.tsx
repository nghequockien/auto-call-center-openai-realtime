...
export default function useRealTime({
    ...
    // Send getting message to the WebSocket server
    const sendGettingMessage = (customerName: string, intent: string) => {
        const command: GettingMessage = {
            type: "session.getting",
            intent: intent,
            user_info: { name: customerName, dob: "1990-01-01", policy_number: "ABC123456" }
        };
        sendJsonMessage(command);
    };
    ...
    const onMessageReceived = (event: MessageEvent<any>) => {
        ...
        switch (message.type) {
            ...
            case "response.audio_transcript.done":
                onReceivedResponseAudioTranscriptDone?.(message as ResponseAudioTranscriptDone);
                break;
    ...
    return { startSession, addUserAudio, inputAudioBufferClear, sendGettingMessage };
}
